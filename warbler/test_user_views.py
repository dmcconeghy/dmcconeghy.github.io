...
# Now we can import app & the current user key
...
# Skip using CSRF in WTF internal testing
...
class UserViewTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.u4 = User.signup("backuptester", "test4@test.com", "password", None)
    ...
    def test_users_index(self):
        with self.client as c:
            ...
            self.assertIn("@backuptester", str(resp.data))
    ...
    def setup_likes(self):
        m1 = Message(text="First warble", user_id=self.testuser_id)
        m2 = Message(text="Second warble", user_id=self.testuser_id)
        m3 = Message(id=9999, text="Final warble", user_id=self.u1_id)
    ...
    def test_user_show_with_likes(self):
        ...
        with self.client as c:
            ...
            self.assertEqual(len(found), 4)
             ...
             # test for a count of 2 messages (m1, m2)
            ...
            # Test for a count of 1 like (m3)
            self.assertIn("1", found[3].text)

    def test_add_like(self):
        m = Message(id=1492, text="The earth is round", user_id=self.u1_id)
        db.session.add(m)
        db.session.commit()
        ...
        with self.client as c:
            ...
            resp = c.post("/messages/1492/like", follow_redirects=True)
            self.assertEqual(resp.status_code, 200)

            likes = Likes.query.filter(Likes.message_id==1492).all()
    ...
    def test_remove_like(self):
        ...
        m = Message.query.filter(Message.text=="Final warble").one()
    ...
    def test_show_following(self):
        ...
        with self.client as c:
            ...
            self.assertIn("@user2", str(resp.data))
            self.assertNotIn("@user3", str(resp.data))
            self.assertNotIn("@backuptester", str(resp.data))
