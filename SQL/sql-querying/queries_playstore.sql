...
-- 1Find the app with an ID of 1880.
SELECT * FROM analytics WHERE id = 1880;
-- 2Find the ID and app name for all apps that were last updated on August 01, 2018.
SELECT id, app_name FROM analytics WHERE last_updated = 'August 01, 2018';
-- 3Count the number of apps in each category, e.g. “Family | 1972”.
SELECT category, COUNT(*) FROM analytics GROUP BY category;
-- 4Find the top 5 most-reviewed apps and the number of reviews for each.
SELECT app_name, reviews FROM analytics ORDER BY reviews DESC LIMIT 5;
-- 5Find the app that has the most reviews with a rating greater than equal to 4.8.
SELECT app_name, MAX(reviews), rating FROM analytics WHERE rating >= 4.8 ORDER BY reviews DESC LIMIT 1;

-- 6Find the average rating for each category ordered by the highest rated to lowest rated.
SELECT SUM(rating)/COUNT(id) as average, category FROM analytics GROUP BY category ORDER BY category, average DESC; 
